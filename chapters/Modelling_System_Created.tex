...
\begin{pyglist}[language = python, encoding = utf8, numbers=left, listingname=\textbf{Code sample}, caption={Example high-level flow from a series of functions}]
...
\begin{pyglist}[language = python, encoding = utf8, numbers=left, listingname=\textbf{Code sample}, caption={Fine grained view of the earlier top-level workflow}]
def implement_feature():
    make_new_feature()
    create_test_and_code()
    unit_test()
...
\begin{pyglist}[language = python, encoding = utf8, numbers=left, listingname=\textbf{Code sample}, caption={Sample from a model including flows and atoms}]
...
\todo{Write about the decorators for flows and atoms that run based on preconditions!}
\todo{Write about preconditions and aftereffects}
